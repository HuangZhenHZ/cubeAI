	...
	void back(double f[]){
		...
		for(int i=n-1; i>=0; --i){
			for(int j=l[i]; j<r[i]; ++j){
				...
				double dx = fout[i] * d[j];
				ws[i][j-l[i]] += dx*dx;
				w[i][j-l[i]] -= dx / sqrt(ws[i][j-l[i]] + 1);
			}
			d[i] *= fout[i] * (1-fout[i]);
			//c[i] -= d[i] * 0.5;
			double dx = d[i];
...
double train(){
	...
	for(int x=1; x<=r; ++x){
		...
		//printf("%d\n",frtt[l-1]^1);
		fout[frtt[x]^1]=1;
		//fout[12]=0.5+0.5/sqrt(dep[x]);
		
		nn.set_input(fin);
		nn.calc();
		totloss+=nn.loss(fout);
		nn.back(fout);
		/*
		int mi=0;
		for(int i=1; i<12; ++i){
			if (nn.fout[i]>nn.fout[mi]) mi=i;
		}
		sum += ( mi == (frtt[x]^1) );
		nn.back(fout);
		*/
